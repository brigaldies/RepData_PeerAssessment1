...
The loaded data contains **`r observations_count`** observations across **`r variables_count`** variables: **`r paste(names(data), collapse=', ')`**.
...
The mean and median (rounded to 2 decimals) of the total number of steps per day are **`r steps_mean_formatted`** and **`r steps_median_formatted`** respectively.
...
1. The time series is plotted with the `ggplot2` `geom_line` function (See Fig. `r fig_num`).
...
**`r steps_na_count_formatted`** rows have a missing `steps` variable value.
...
With the missing data imputed, the new mean and median of the total number of steps per day are **`r steps_mean_formatted`** and **`r steps_median_formatted`** respectively. By comparison with the [original](#histogram-of-total-number-of-steps-taken-per-day) mean **`r steps_mean_no_na_formatted`** and median **`r steps_median_no_na_formatted`** with the missing data removed, the following observations are made:
...
```{r hist_of_total_steps_per_day_na_imputed, fig.width=8}    
...
```{r weekdays_vs_weekends_activity}
...
Figure `r fig_num` plots the time series of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all days (y-axis), on two vertical panels to compare between weekdays and weekend days.

```{r weekdays_vs_weekends_activity_plot, fig.width=8}        
# Plot
plot <- 
